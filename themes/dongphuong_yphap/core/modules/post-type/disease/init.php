<?php
require_once dirname(__FILE__) . '/disease-meta-box.php';
if (!function_exists('wb_create_post_type_disease')) {
	function wb_create_post_type_disease()
	{
		if (current_user_can('user_aprove_comments'))
			return;
		$labels  = array(
			'name'                  => _x('Bệnh học', 'Post Type Name', 'dongphuong_yphap'),
			'singular_name'         => _x('Bệnh học', 'Post Type Singular Name', 'dongphuong_yphap'),
			'menu_name'             => __('Bệnh học', 'dongphuong_yphap'),
			'name_admin_bar'        => __('Bệnh học', 'dongphuong_yphap'),
			'archives'              => __('Tất cả Bệnh học', 'dongphuong_yphap'),
			'parent_item_colon'     => __('Parent Item:', 'dongphuong_yphap'),
			'all_items'             => __('Tất cả Bệnh học', 'dongphuong_yphap'),
			'add_new_item'          => __('Thêm Bệnh học', 'dongphuong_yphap'),
		...
		$args    = array(
			'label'               => __('Bệnh học', 'dongphuong_yphap'),
			'description'         => __('Tổng hợp những Bệnh học', 'dongphuong_yphap'),
			'labels'              => $labels,
	...
	add_action('init', 'wb_create_post_type_disease', 0);
}
